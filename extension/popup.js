...
function analyzeCurrentImage() {
    ...
    // Show loading spinner and button loading state
    ...
    chrome.runtime.sendMessage(
        ...
        (response) => {
            // Hide loading spinner and button loading state
...
function loadHistory() {
    ...
    // Add loading state to history button
    const historyButtonLoading = document.getElementById("back-to-analysis");
    historyButtonLoading.classList.add("btn-loading");

    // Send get history request to background script
    chrome.runtime.sendMessage({ action: "getHistory" }, (response) => {
        try {
            // Remove loading element and button loading state
            if (historyContainer.contains(loadingElement)) {
                historyContainer.removeChild(loadingElement);
            }

            // Remove loading state from history button
            const historyButtonLoading =
                document.getElementById("back-to-analysis");
            historyButtonLoading.classList.remove("btn-loading");
